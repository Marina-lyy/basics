...
### String对象原型
    ...
    slice（） 切割字符串并且返回部分字符串
    ·  格式：结果变量 = 字符串变量.slice（开始位置【，结束位置】）
    ·  位置参数，可以正数，可以负数，正数从前数，负数从后数
    
    split（） 使用指定的字符切割字符串，并将结果使用数组返回
    ·  格式：结果变量 = 字符串变量.split(指定字符）；
    ·  注意：该方法和数组中的join方法是一个相反的过程
    
    replace（） 字符串替换函数
    ·  格式：结果变量 = 字符串变量.replice(要查找的字符串，替换字符串）
    
    toUpperCase() 将字符串转化为大写
    ·  格式：结构变量 = 字符串变量.toUpperCase();
    
    toLowerCase（） 将字符串转化为小写
    ·  格式：结构变量 = 字符串变量.toLowerCase();
    案例js29
    ...
    search（）进行正则查找，将查找到的第一个符合的字符串的位置返回
    ·  格式：结果变量 = 字符串变量.search（正则表达式）
    
    substr() 返回指定位置的指定长度字符串
    ...
    ·  注意：
    ·      位置参数都必须为0 或者正整数，负数位置表示0
    ·      第二个参数也是位置参数
    ·      截取字符串时，从最小位置参数开始，截取到最大位置参数之前，不包含最大位置
